...
impl Grid {
    ...
    /// euclidian distance between two nodes on a grid. Uses their row/col index as x/y position
    ...
    fn print_grid(&self) {
        for row in &self.grid {
            for node in row {
                if self.cur_path.contains(node) && node.ntype == NodeType::Empty {
                    print!(" *");
                } else {
                    print!(" {}", node);
                }
